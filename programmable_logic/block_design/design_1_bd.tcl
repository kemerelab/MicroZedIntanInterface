...
# module references:
# simple_dual_port_bram_wrapper, led_status_controller, axi_lite_registers, data_generator, intan_spi_lvds_buffer
...
\
  ...
  set_property -dict [list CONFIG.NUM_SI {2} CONFIG.NUM_MI {5}] $smartconnect_1
  ...
  # Create instance: simple_dual_port_bram_lfp -- the LFP/DSP engine output BRAM.
  # PL write side = data_generator/LFP_BRAM; PS read side = axi_bram_ctrl_1.
  set block_name simple_dual_port_bram_wrapper
  set block_cell_name simple_dual_port_bram_lfp
  if { [catch {set simple_dual_port_bram_lfp [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>."}
     return 1
   } elseif { $simple_dual_port_bram_lfp eq "" } {
  ...
  # Create instance: simple_dual_port_bram_playback -- the synthetic-data playback
  # BRAM (256 KB). PS write side = axi_bram_ctrl_3; PL read side = data_generator.
  set block_name playback_bram_wrapper
  set block_cell_name simple_dual_port_bram_playback
  if { [catch {set simple_dual_port_bram_playback [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>."}
     return 1
   } elseif { $simple_dual_port_bram_playback eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>."}
     return 1
   }

  # Create instance: axi_bram_ctrl_3 -- PS write port for the playback BRAM
  ...
  # Create instance: intan_spi_lvds_buffer_0, and set properties
  set block_name intan_spi_lvds_buffer
  set block_cell_name intan_spi_lvds_buffer_0
  if { [catch {set intan_spi_lvds_buffer_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $intan_spi_lvds_buffer_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  # LFP output BRAM: PL write side (data_generator/LFP_BRAM) + PS read side (axi_bram_ctrl_1)
  ...
  # Playback BRAM: PL read side (data_generator/PLAYBACK_BRAM) + PS write side (axi_bram_ctrl_3)
  connect_bd_intf_net -intf_net axi_bram_ctrl_3_BRAM_PORTA [get_bd_intf_pins axi_bram_ctrl_3/BRAM_PORTA] [get_bd_intf_pins simple_dual_port_bram_playback/BRAM_PORTB]
  connect_bd_intf_net -intf_net data_generator_PLAYBACK_BRAM [get_bd_intf_pins data_generator/PLAYBACK_BRAM] [get_bd_intf_pins simple_dual_port_bram_playback/BRAM_PORTA]
  connect_bd_intf_net -intf_net smartconnect_1_M04_AXI [get_bd_intf_pins smartconnect_1/M04_AXI] [get_bd_intf_pins axi_bram_ctrl_3/S_AXI]
  ...
  # Playback BRAM, PS write view @ 0x8C000000 (128 KB)
  assign_bd_address -offset 0x8C000000 -range 0x00020000 -target_address_space [get_bd_addr_spaces processing_system7_0/Data] [get_bd_addr_segs axi_bram_ctrl_3/S_AXI/Mem0] -force
  ...
  # let the CDMA also read the STFT results BRAM (BRAM->DDR for the spectrum stream)
  assign_bd_address -offset 0x88000000 -range 0x00010000 -target_address_space [get_bd_addr_spaces axi_cdma_0/Data] [get_bd_addr_segs axi_bram_ctrl_2/S_AXI/Mem0] -force
