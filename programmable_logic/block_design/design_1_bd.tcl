...
\
  ...
  # M02 -> axi_bram_ctrl_1 (LFP BRAM @0x84000000); M03 -> axi_bram_ctrl_2 (accel/movement BRAM @0x88000000)
  ...
  # Create instance: simple_dual_port_bram_accel -- the movement accel-extract
  # output BRAM. PL write side = data_generator/ACCEL_BRAM; PS read side = axi_bram_ctrl_2.
  set block_name simple_dual_port_bram_wrapper
  set block_cell_name simple_dual_port_bram_accel
  if { [catch {set simple_dual_port_bram_accel [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>."}
     return 1
   } elseif { $simple_dual_port_bram_accel eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>."}
     return 1
   }

  # Create instance: axi_bram_ctrl_2 -- PS read port for the accel output BRAM
  ...
  # Accel output BRAM: PL write side (data_generator/ACCEL_BRAM) + PS read side (axi_bram_ctrl_2)
  connect_bd_intf_net -intf_net axi_bram_ctrl_2_BRAM_PORTA [get_bd_intf_pins axi_bram_ctrl_2/BRAM_PORTA] [get_bd_intf_pins simple_dual_port_bram_accel/BRAM_PORTB]
  connect_bd_intf_net -intf_net data_generator_ACCEL_BRAM [get_bd_intf_pins data_generator/ACCEL_BRAM] [get_bd_intf_pins simple_dual_port_bram_accel/BRAM_PORTA]
  ...
  # Accel/movement output BRAM, PS read view
  ...
  # Accel/movement BRAM in the CDMA master view (read src -- the PL builds the whole
  # block there and the PS CDMAs it; REQUIRED or a CDMA of the accel BRAM hangs).
