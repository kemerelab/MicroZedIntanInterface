...
def main():
    ...
    #      most-recent column of every active (octave, voice). The VALUES are
    #      identical to the v2 engine (bit-exact). We keep BOTH groupings:
    #        payload[]   : lane-major / octave / voice (the legacy whole-surface
    #                      order -- used by the values-only bit-exact check)
    #        oct_pay[o]  : per-octave packet payload (lane-major / voice-minor
    #                      for THAT octave) -- the STEP 2 unified per-octave wire
    #                      packet payload. ----
    payload = []                                  # legacy whole-surface order
    oct_pay = [[] for _ in range(ACT_OCTAVES)]    # per-octave packet payloads
    for l in range(K):
        ...
        for o in range(ACT_OCTAVES):
            ...
            for (re, im) in col:
                payload.append(re)
                payload.append(im)
                oct_pay[o].append(re)            # per-octave: lane-major/voice-minor
                oct_pay[o].append(im)

    # ---- legacy values-only reference (lane/octave/voice order). The values
    #      MUST be unchanged from the v2 engine -- this is the bit-exact gate. ----
    with open(f"{OUT}/wav_exp.hex", "w") as f:
        for v in payload:
            f.write(format(v & 0xFFFFFFFF, '08x') + "\n")

    # ---- STEP 2: per-octave UNIFIED wire packets. One file per octave, each an
    #      8-word common header (type=3) + this octave's K*ACT_VOICES complex bins
    #      (lane-major/voice-minor). The TB snapshots each octave's BRAM region
    #      and compares it. SEQ/timestamp are runtime (advance counts), so the TB
    #      checks them structurally (nonzero/advancing), not against a fixed value;
    #      MAGIC/TYPE_VER/AUX0/AUX1 are exact. ----
    UNIFIED_MAGIC = 0xCAFEBABE
    STREAM_TYPE_WAV = 3
    UNIFIED_VERSION = 1
    type_ver = (STREAM_TYPE_WAV & 0xFF) | ((UNIFIED_VERSION & 0xFF) << 8)
    for o in range(ACT_OCTAVES):
        aux0 = (o & 0xF) | ((ACT_OCTAVES & 0xF) << 4) | ((ACT_VOICES & 0xF) << 8)  # overrun=0
        aux1 = (K & 0xFF)                                # lane_start=0, n_channels=K
        # header w2/w3 (timestamp) and w4 (seq) are runtime -> emit 0 placeholders;
        # the TB does NOT compare those two header words exactly.
        header = [UNIFIED_MAGIC, type_ver, 0, 0, 0, aux0, aux1, 0]
        wire = header + oct_pay[o]
        with open(f"{OUT}/wav_oct{o}_exp.hex", "w") as f:
            for v in wire:
                f.write(format(v & 0xFFFFFFFF, '08x') + "\n")
    ...
    per_oct_words = HDR_WORDS + K * ACT_VOICES * 2
    print(f"K={K} build(oct={N_OCTAVES},V={V},taps={N_TAPS}) "
          f"active(oct={ACT_OCTAVES},V={ACT_VOICES},taps={ACT_TAPS}) hb={HB_TAPS} "
          f"frames={N_FRAMES} -> per-octave packet = {HDR_WORDS} hdr + "
          f"{K*ACT_VOICES*2} payload = {per_oct_words} words "
          f"(K*V bins, re/im interleaved); {ACT_OCTAVES} octave packets")
