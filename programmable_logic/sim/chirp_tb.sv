...
// data_generator_core. Drives the core in debug_mode + chirp_mode, captures the
// data word at each (packet, cycle_slot) via the DSP tap, and compares the 8
// lane sine values against the Python NCO reference (gen_chirp_vectors.py).
//
...
module chirp_tb;
    ...
    logic [7:0]       mask;
    logic             csn, sclk, copi;
    logic             dsp_valid, dsp_tick;
    logic [127:0]     dsp_data;
    logic [5:0]       dsp_slot;
    ...
    data_generator_core dut (
        ...
        .digital_in(8'h00),
        .dsp_sample_valid(dsp_valid), .dsp_sample_data(dsp_data),
        .dsp_sample_slot(dsp_slot), .dsp_packet_tick(dsp_tick)
    );

    logic [127:0] exp_w [0:NW-1];

    // capture: index by (packet, cycle). The DSP tap pulses once per cycle slot
    // (state 77) carrying the data word; record per-packet rows in order.
    logic [127:0] got_w [0:NW-1];
    int n_got = 0;
    always @(posedge clk) if (rstn && dsp_valid && n_got < NW) begin
        got_w[n_got] = dsp_data;
        n_got++;
    end
