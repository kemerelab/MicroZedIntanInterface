...
def write_sv_defaults(path, hb_q, lin_q):
    """Power-on defaults so the engine filters correctly with no host upload."""
    def arr(q):
        mask = (1 << COEF_W) - 1
        return ",\n        ".join(
...
//
// Power-on default coefficients for the LFP decimation chain, Q1.{COEF_FRAC} signed.
// The host can overwrite either stage at runtime; these are what the board
// filters with straight out of reset.
//
//   stage 1: {len(hb_q)}-tap halfband, 30 kHz -> 15 kHz
//   stage 2: {len(lin_q)}-tap linear phase, 15 kHz -> 3 kHz (passband to {F_PASS:.0f} Hz)
//
// The minimum-phase stage-2 alternative is in lfp_poly120_min_coefs.hex; upload
// it from the host when latency matters more than constant group delay.
...
def main():
    ...
    write_sv_defaults(os.path.join(args.outdir, "lfp_filter_defaults.svh"), hb_q, lin_q)
    print("\nwrote " + ", ".join(fn for fn, _ in outs) + ", lfp_filter_defaults.svh")
