...
// three NAMED things -- two command registers and one cycling program:
//
//   slot 0 (cycle 32, AUX_FS_SLOT)     : a fixed command register, rewritten LIVE
//                                        by the override (fast-settle whole-replace
//                                        + Reg-3 digout). Default CONVERT(32).
//   slot 1 (cycle 33, AUX_PLAIN_SLOT)  : the ONE cycling program (aux_program) --
//                                        the accelerometer / aux-ADC sweep. This is
//                                        the only banked sequencer in the design.
//   slot 2 (cycle 34, AUX_INJECT_SLOT) : a fixed command register, whole-replaced
//                                        for ONE packet by a one-shot register
//                                        READ/WRITE injection (control plane; the
//                                        reply is captured downstream). Default
//                                        CONVERT(34).
//
...
module aux_command_engine #(
    ...
    // Status: only slot 1 (the program) has a bank/index; slots 0 and 2 report 0.
...
// ===========================================================================
// reg 22 (aux_ctrl_reg): [1] slot-1 program bank select, [4] fs_sw, [5] fs_gpio_en,
...
wire        prog_bank_select = aux_ctrl_reg[1];   // only slot 1 cycles -> one bank bit
...
// ===========================================================================
// slots 0 and 2 -- fixed command registers (no bank, no index). Each is set via
// the write port; slot 0 is rewritten by the override, slot 2 by injection.
// A length write to either target is a no-op (registers have no length).
// ===========================================================================
logic [15:0] rt_cmd_r;        // slot 0 base (override-rewritten)
...
always_ff @(posedge clk) begin
    if (!rstn) begin
        ...
        if (wr_target == 2'(AUX_FS_SLOT))     rt_cmd_r      <= wr_data;
        if (wr_target == 2'(AUX_INJECT_SLOT)) inject_base_r <= wr_data;
    end
end

// ===========================================================================
// slot 1 -- the one cycling program. This is the ONLY banked-sequencer machinery.
// ===========================================================================
logic [15:0]       prog_cmd;
...
assign seq_cmds[AUX_PLAIN_SLOT*16  +: 16] = prog_cmd;                                    // slot 1
...
// Status: only slot 1 (the program) has a bank/index; the two register slots
// report 0 (they neither bank-swap nor cycle).
assign bank_active = {1'b0, prog_bank, 1'b0};   // [2]=slot2, [1]=slot1, [0]=slot0
assign slot_indices[AUX_FS_SLOT*ADDR_W     +: ADDR_W] = '0;
assign slot_indices[AUX_PLAIN_SLOT*ADDR_W  +: ADDR_W] = prog_index;
