...
module wavelet_dsp_block #(
    ...
    parameter int K         = 40,   // bounded so a 1-octave packet fits 1 datagram
                                    // (K*V <= 180 at V=4 -> K=40 = 160 OK)
    ...
    wire [7:0]  n_chan8   = wav_cfg[27:20];   // active streamed channels (1..K)
    ...
    wire [LANE_W:0]  n_chan_cfg    = (n_chan8 == 0) ? (LANE_W+1)'(K)
                                   : (n_chan8 > K) ? (LANE_W+1)'(K) : (LANE_W+1)'(n_chan8);
    ...
    wavelet_cqt_engine #(
        ...
        .master_timestamp(master_timestamp), .n_channels_cfg(n_chan_cfg),
