...
// Merge of main (chirp, +8B) and tier3 (wavelet, +20B): 160 base + 8 + 20 = 188.
_Static_assert(sizeof(status_response_t) == 188, "status_response_t size must match net.py get_status");
...
void network_maintenance_loop(void) {
  ...
  // Ship the wavelet scalogram surface -> UDP (Tier-3). No-op unless the
  // engine is enabled; polls the column counter and sends on advance.
  wav_stream_service();
...
int main() {
  ...
  wav_stream_init();   // wavelet scalogram monitor stream (port 5004)
